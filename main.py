...
# Android-friendly display. The game uses an internal 800x600 canvas
# and pygame scales it to the phone display.
...
# APK assets are read-only. Android progress must live in writable
# application storage so it survives closing and reopening the game.
def get_app_data_dir():
    candidates = [
        os.environ.get("ANDROID_PRIVATE"),
        os.environ.get("ANDROID_APP_PATH"),
        os.path.join(os.path.expanduser("~"), ".cosmic_defender"),
    ]
...
SAVE_FILE = writable_path("cosmic_progress.json")
LEADERBOARD_FILE = writable_path("leaderboard.json")
MUSIC_FILE = writable_path("space_music.wav")
...
def install_default_save_files():
    # Copy the supplied files only when the writable copy does not exist.
    # Existing leaderboard entries and progress are never overwritten.
    for filename in ("cosmic_progress.json", "leaderboard.json"):
...
def screen_to_game_pos(pos):
    """Map physical/touch coordinates to the game's 800x600 space."""
    try:
...
def move_player_to_touch(pos):
    gx, gy = screen_to_game_pos(pos)
    player.centerx = max(35, min(WIDTH - 35, gx))
    player.centery = max(HEIGHT - 155, min(HEIGHT - 55, gy))
...
while running:
    ...
    for event in pygame.event.get():
        ...
        # Save immediately when Android sends the app to the background.
        ...
        # Android back button is normally delivered as ESC.
        elif event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE and IS_ANDROID:
            if player_name:
                save_position_now()
            if game_state == "playing":
                game_state = "paused"
            elif game_state in ("paused", "leaderboard", "level_complete", "gameover", "final"):
                game_state = "menu"
        ...
        # Touch controls: drag the ship; tap to fire.
        elif event.type in (pygame.FINGERDOWN, pygame.FINGERMOTION) and game_state == "playing":
            touch_pos = (event.x * WIDTH, event.y * HEIGHT)
            move_player_to_touch(touch_pos)
        ...
        # ====================================================
            ...
            if event.type == pygame.TEXTINPUT:
                ...
                if len(name_input) < 20:
                    name_input += event.text[:20 - len(name_input)]
                ...
                mouse = (pygame.mouse.get_pos() if event.type == pygame.MOUSEBUTTONDOWN else (int(event.x * WIDTH), int(event.y * HEIGHT)))

                name_box = pygame.Rect(WIDTH // 2 - 180, 215, 360, 60)
                if name_box.collidepoint(mouse):
                    try:
    ...
    if game_state == "playing":
        ...
        # ----------------------------------------------------
        # AUTO SAVE (avoid writing the JSON file every frame)
        # ----------------------------------------------------
